...
//   const { options, updateOptions, addOption } = useDropdown();
...
const DraggableDropdown = () => {
  const { dropdownOptions, updateOptions, addOption } = useDropdown();
  const [internalOptions, setInternalOptions] = useState([]);
  const [editingId, setEditingId] = useState(null);
  const [inputValue, setInputValue] = useState("");
  const inputRef = useRef(null);
  ...
  useEffect(() => {
    setInternalOptions(dropdownOptions);
  }, [dropdownOptions]);
  ...
  const handleDragEnd = (event) => {
    ...
    if (active?.id && over?.id && active.id !== over.id) {
      ...
      const newOptions = arrayMove(internalOptions, oldIndex, newIndex);
      setInternalOptions(newOptions);
      updateOptions(newOptions); // Update the context and local storage
    }
  };
